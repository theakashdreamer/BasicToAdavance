...
public final class NetworkMonitorII {

    public static final class Status {
        public enum Type { AVAILABLE, UNAVAILABLE, LOSING, LOST }
        public final Type type;
        @Nullable public final Integer maxMsToLive;
        private Status(Type t, @Nullable Integer ms) { this.type = t; this.maxMsToLive = ms; }
        ...
        @Override public String toString() { return type == Type.LOSING ? "LOSING(" + maxMsToLive + "ms)" : type.name(); }
    }

    private static volatile NetworkMonitorII INSTANCE;

    public static NetworkMonitorII getInstance() {
        if (INSTANCE == null) {
            synchronized (NetworkMonitorII.class) {
                if (INSTANCE == null) INSTANCE = new NetworkMonitorII();
            }
        }
        return INSTANCE;
    }
    ...
    private NetworkMonitorII() {
        isConnectedLive.addSource(statusLive, s -> isConnectedLive.setValue(s.type == Status.Type.AVAILABLE));
    }

    private final ConnectivityManager.NetworkCallback callback = new ConnectivityManager.NetworkCallback() {
        @Override public void onAvailable(@NonNull Network network) { updateFromCapabilities(network); }

        @Override public void onCapabilitiesChanged(@NonNull Network network, @NonNull NetworkCapabilities caps) {
            boolean connected = isLanConnected(caps);
            statusLive.postValue(connected ? Status.available() : Status.unavailable());
        }

        @Override public void onLosing(@NonNull Network network, int maxMsToLive) {
        ...
        @Override public void onLost(@NonNull Network network) {
            ...
            statusLive.postValue(currentHasInternet() ? Status.available() : Status.unavailable());
        }
    ...
    private boolean currentHasInternet() {
        ...
        return isLanConnected(caps);
    }

    private boolean isLanConnected(@NonNull NetworkCapabilities caps) {
        ...
        boolean hasInternetCap = caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET);
        boolean isValidated = hasValidatedInternet(caps);

        // Treat LAN connected if Ethernet or WiFi transport exists,
        // and either validated internet or at least internet capability is present
        return (hasEthernet || hasWifi) && (isValidated || hasInternetCap);
    }
    ...
    private void updateFromCapabilities(@NonNull Network network) {
        ...
        statusLive.postValue(isLanConnected(caps) ? Status.available() : Status.unavailable());
    }
}
